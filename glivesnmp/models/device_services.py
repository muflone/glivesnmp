...
class ModelDeviceServices(ModelAbstract):
    COL_STATUS = 1
    ...
    def set_data(self, treeiter, item):
        ...
        self.model.set_value(treeiter, self.COL_STATUS, item.status)

    def get_status(self, treeiter):
        """Get the status from a TreeIter"""
        return self.model[treeiter][self.COL_STATUS]

    def set_status(self, treeiter, status):
        """Set the status of a TreeIter"""
        self.model[treeiter][self.COL_STATUS] = status
